...
// Fallback when a product image URL 404s (e.g. deleted Cloudinary asset)
const PLACEHOLDER_IMG = "https://placehold.co/600x800/f3f4f6/a1a1aa?text=No+Image";
...
export default function ProductDetailsClient() {
    ...
    // Size guide uses a native <dialog> opened imperatively via ref — this
    // sidesteps React state-timing/portal edge cases entirely. showModal()/close()
    // are direct DOM calls, not dependent on a re-render happening correctly.
    ...
    // Track + fetch recently-viewed products (localStorage, newest first, max 8)
    useEffect(() => {
        if (!product?._id) return;
        let ids: string[] = [];
        try { ids = JSON.parse(localStorage.getItem("shubhlaxmi_recently_viewed") || "[]"); } catch { ids = []; }

        const otherIds = ids.filter(id => id !== product._id);
        localStorage.setItem("shubhlaxmi_recently_viewed", JSON.stringify([product._id, ...otherIds].slice(0, 9)));

        if (otherIds.length === 0) { setRecentlyViewed([]); return; }
        fetch(`${API_BASE}/public/products`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ ids: otherIds.slice(0, 8) })
        })
            .then(r => r.json())
            .then(d => {
                if (d.success && Array.isArray(d.data)) {
                    // Preserve most-recent-first order
                    const byId = new Map(d.data.map((p: any) => [p._id, p]));
                    setRecentlyViewed(otherIds.map(id => byId.get(id)).filter(Boolean));
                }
            })
            .catch(() => { });
    }, [product?._id]);
    ...
    const kidsRows = [
        ["1–2 Yrs", "Up to 85", "20", "21"],
        ["3–4 Yrs", "86–95", "22", "23"],
        ["5–6 Yrs", "96–105", "24", "25"],
        ["7–8 Yrs", "106–115", "26", "27"],
        ["9–10 Yrs", "116–125", "28", "29"],
        ["11–12 Yrs", "126–135", "30", "31"],
    ];

    const sizeGuideTable = isKids
        ? { title: "Kids Size Chart", headers: ["Age Group", "Height (cm)", "Chest (Inch)", "Length (Inch)"], rows: kidsRows }
        : isMens
            ? { title: "Men's Size Chart", headers: ["Size", "Chest (Inch)", "Waist (Inch)", "Hip (Inch)"], rows: menRows }
            : { title: "Women's Size Chart", headers: ["Size", "Bust (Inch)", "Waist (Inch)", "Hip (Inch)"], rows: womenRows };

    // Always mounted (not conditionally created) — a native <dialog> opened via
    // showModal()/close() on the ref. This is immune to any React re-render /
    // portal-timing edge case: the element always exists, only its open state changes.
    const renderSizeGuideModal = () => (
        <dialog
            ref={sizeGuideRef}
            aria-label="Size guide"
            className="rounded-xl shadow-2xl p-0 w-full max-w-lg m-auto backdrop:bg-black/50 backdrop:backdrop-blur-sm"
            onClick={(e) => { if (e.target === sizeGuideRef.current) closeSizeGuide(); }}
        >
            <div className="p-6 sm:p-8 max-h-[85vh] overflow-y-auto">
                ...
                <h3 className="text-xl font-serif mb-1">{sizeGuideTable.title}</h3>
                <p className="text-xs text-gray-500 mb-5">All measurements are body measurements in inches unless noted.</p>

                <div className="overflow-x-auto">
                    <table className="w-full text-sm border-collapse">
                        <thead>
                            <tr className="bg-pink-50 text-[var(--brand-pink)]">
                                {sizeGuideTable.headers.map(h => (
                                    <th key={h} className="border border-pink-100 px-3 py-2 text-left font-semibold">{h}</th>
                                ))}
                            </tr>
                        </thead>
                        <tbody>
                            {sizeGuideTable.rows.map((row, i) => (
                                <tr key={i} className={i % 2 ? "bg-gray-50" : "bg-white"}>
                                    {row.map((cell, j) => (
                                        <td key={j} className={`border border-gray-100 px-3 py-2 ${j === 0 ? "font-semibold text-gray-800" : "text-gray-600"}`}>{cell}</td>
                                    ))}
                                </tr>
                            ))}
                        </tbody>
                ...
                <div className="mt-6 text-[13px] text-gray-600 space-y-2 bg-gray-50 rounded-lg p-4">
                    <p className="font-semibold text-gray-800">How to measure</p>
                    <p>• <strong>Bust/Chest:</strong> Measure around the fullest part, keeping the tape level.</p>
                    <p>• <strong>Waist:</strong> Measure around the natural waistline.</p>
                    <p>• <strong>Hip:</strong> Measure around the fullest part of the hips.</p>
                    <p className="pt-1 text-gray-500">Between sizes? Choose the larger one, or select "Custom Size" and our Tailor Master will contact you.</p>
                </div>
            </div>
        </dialog>
    );
        ...
        <div className="bg-white min-h-screen">
            ...
            <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-16 flex flex-col lg:flex-row lg:items-start gap-8 lg:gap-14">
                ...
                <div className="w-full lg:w-[55%] flex flex-col-reverse lg:flex-row gap-4">
                    ...
                    <div className="flex lg:flex-col gap-3 overflow-x-auto lg:overflow-y-auto lg:w-24 shrink-0 no-scrollbar pb-2 lg:pb-0">
                        {product.images && product.images.map((img: any, idx: number) => (
                            ...
                            >
                                <img src={img.url} onError={onImgError} alt={`${product.title} view ${idx + 1}`} className="w-full h-full object-cover" />
                            </button>
                        ))}
                    </div>
                    {/* Main Image */}
                    <div className="flex-1 rounded-sm overflow-hidden bg-gray-50 aspect-[3/4]">
                        {selectedImage ? (
                            <img src={selectedImage} onError={onImgError} alt={product.title} className="w-full h-full object-cover" />
                        ) : (
                ...
                <div className="w-full lg:w-[45%] flex flex-col pt-2 lg:pt-0">
                    ...
                    {/* relative z-10: the negative margin below used to be -mb-3, which
                        pulled this row down over the sizing fields' first label — later
                        DOM siblings paint above earlier ones when they overlap in normal
                        flow, so every click landed on the label instead of this button.
                        Kept z-10 as a safety net even with the margin now fixed. */}
                    <div className="relative z-10 flex justify-end mb-2">
                        <button
                            type="button"
                            onClick={openSizeGuide}
                            className="text-xs font-semibold tracking-wider uppercase text-[var(--brand-pink)] underline underline-offset-4 hover:text-[#d61b7f] transition-colors"
                        >
                            📏 Size Guide
                        </button>
                    </div>
                    ...
                    <div className="mb-10">
                        ...
                        <div className="mt-6 flex flex-col gap-3">
                            ...
                            {/* Card badges only make sense when on-site card payment is enabled */}
                            {!settings.whatsappCheckoutEnabled && (
                        ...
                        {relatedProducts.map((p: any) => (
                            <Link href={`/product/${p.slug}`} key={p._id} className="group flex flex-col cursor-pointer">
                                <div className="relative aspect-[3/4] overflow-hidden bg-gray-100 rounded-sm mb-4">
                                    <img
                                        src={p.images?.[0]?.url || PLACEHOLDER_IMG}
                                        onError={onImgError}
                                        alt={p.title}
            ...
            {recentlyViewed.length > 0 && (
                <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 pb-16 lg:pb-24">
                    <h2 className="text-2xl lg:text-3xl font-[var(--font-heading)] text-center mb-12 text-gray-900 font-medium">Recently Viewed</h2>
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-6 lg:gap-8">
